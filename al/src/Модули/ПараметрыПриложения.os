...
Функция ПолнаяВерсияПродукта() Экспорт
	
	Возврат СтрШаблон("Автоматизированная утилита повседневной деятельности %1 %2", ИмяПродукта(), ВерсияПродукта());
...
Функция ИмяЛогаСистемы() Экспорт
	...
	Возврат "auto.oscript" + ИмяПродукта();
...
Функция ИмяКомандыПодкасты() Экспорт
...
Процедура НастроитьКомандыПриложения(Знач Приложение) Экспорт
	...
	Приложение.ДобавитьКоманду(ИмяКомандыПодкасты(), "КомандаПодкасты",  
		        "Добавляет возможности работа с подкастами");
